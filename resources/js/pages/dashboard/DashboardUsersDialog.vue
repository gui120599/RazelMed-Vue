...
const currentDashboardUsers = ref([]);
...
// 3. Função para popular currentDashboardUsers.value
// Esta função precisa ser chamada quando a prop 'dashboard' muda OU quando o diálogo abre.
const updateDashboardUsers = () => {
    // Garanta que `dashboard.users` esteja carregado no backend.
    // O Inertia.js garante que `props.dashboard` já está atualizado
    // se o `router.reload({ only: ['dashboards'] })` for bem-sucedido.
    currentDashboardUsers.value = props.dashboard.users || [];
};
...
// 5. Watch para mudanças na prop 'dashboard'.
// Isso é crucial porque 'dashboard' pode ser atualizada após um router.reload.
// O { deep: true } garante que mudanças nas propriedades aninhadas (como 'users') sejam detectadas.
watch(() => props.dashboard, (newVal, oldVal) => {
...
const usersWithAccess = computed(() => {
    ...
    return currentDashboardUsers.value.filter(user =>
...
const usersWithoutAccess = computed(() => {
    const usersWithAccessIds = new Set(currentDashboardUsers.value.map(user => user.id));
    const term = searchTerm.value.toLowerCase();
...
const attachUser = (userId) => {
    form.user_id = userId;
    form.post(route('dashboards.users.attach', props.dashboard.id), {
        onSuccess: () => {
            toast.success('Usuário adicionado com sucesso!');
            // Chamar router.reload para atualizar a prop 'dashboards' no parent
            router.reload({ only: ['dashboards'] });
            // O watcher de props.dashboard (com deep: true) + internalOpen.value
            // garantirá que currentDashboardUsers.value seja atualizado.
            // O nextTick é para garantir que a atualização do DOM aconteça
            // após as reatividades do Vue serem processadas.
            nextTick(() => {
                updateDashboardUsers(); // Força a atualização após o reload e a reatividade do Vue
            });
        },
        onError: (errors) => {
...
const detachUser = (userId) => {
    ...
    form.user_id = userId;
    form.delete(route('dashboards.users.detach', props.dashboard.id), {
        onSuccess: () => {
            toast.success('Usuário removido com sucesso!');
            router.reload({ only: ['dashboards'] }); // Recarrega as props da página
            nextTick(() => {
                updateDashboardUsers(); // Força a atualização
            });
        },
        onError: (errors) => {
...
<template>
    <Dialog v-model:open="internalOpen">
        ...
        <DialogContent class="lg:max-w-[1200px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle>Gerenciar Usuários da Instituição: {{ dashboard.name }}</DialogTitle>
                <DialogDescription>
